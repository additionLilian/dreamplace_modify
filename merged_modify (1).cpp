    ...
    int num_threads){
        ...
        for (int i = 0; i < num_nets; ++i){
            if (net_mask[i]){
                ...
                pin_count = pin_count + degree;
                if (pin_count > batch_size_pin){
                    net_list[nets_count] = i;
                    pin_count = 0;
                }
            }
        }
...
int computeWeightedAverageWirelengthMergedLauncher(
    ...
    T *partial_wl, T *grad_intermediate_x, T *grad_intermediate_y,
    int num_threads) {
        int net_list[] = SplitNets(const T *x, const T *y, const int *flat_netpin, const int *netpin_start,
    const unsigned char *net_mask, int num_nets, const T *inv_gamma,
    T *partial_wl, T *grad_intermediate_x, T *grad_intermediate_y,
    int num_threads);

    int num_batch = len(net_list) - 1;
    int rand_number = rand()%(num_batch);
    int net_index[2] = {net_list[rand_number], net_list[rand_number+1]};
    int chunk_size =
...
//++
      partial_wl[i] = (xexp_x_sum / exp_x_sum - xexp_nx_sum / exp_nx_sum +
                      yexp_y_sum / exp_y_sum - yexp_ny_sum / exp_ny_sum)*(num_batch+1);
//++
      ...
      for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j) {
        ...
        grad_intermediate_x[flat_netpin[j]] =
            ((a_x + b_x * xx) * exp_x - (a_nx + b_nx * xx) * exp_nx))*(num_batch+1);
        ...
        grad_intermediate_y[flat_netpin[j]] =
            ((a_y + b_y * yy) * exp_y - (a_ny + b_ny * yy) * exp_ny)*(num_batch+1);
      }
    }
  }
...
// the original version (full gradient)
int computeWeightedAverageWirelengthMergedLauncher2(
    const T *x, const T *y, const int *flat_netpin, const int *netpin_start,
    const unsigned char *net_mask, int num_nets, const T *inv_gamma,
    T *partial_wl, T *grad_intermediate_x, T *grad_intermediate_y,
    int num_threads, int net_index) {
  int chunk_size =
      DREAMPLACE_STD_NAMESPACE::max(int(num_nets / num_threads / 16), 1);
