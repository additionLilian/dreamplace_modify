...
class NesterovAcceleratedGradientOptimizer(Optimizer):
    ...
    def __init__(self, params, lr=required, obj_and_grad_fn=required, obj_and_grad_fn2 = required, obj_and_grad_fn_pin = required, constraint_fn=None):
        """
        ...
        self.obj_and_grad_fn_pin = obj_and_grad_fn_pin
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            #logging.info('self params including %s' %(group))
        ...
        return loss
# ++ add
    def step_pin(self, closure=None):
        """
        @brief Performs a single optimization step.
        @param closure A callable closure function that reevaluates the model and returns the loss.
        """
        ...
        for group in self.param_groups:
            #logging.info('self params including %s' %(group))
            obj_and_grad_fn = self.obj_and_grad_fn_pin
            constraint_fn = self.constraint_fn
            for i, p in enumerate(group['params']):
                if p.grad is None:
                    continue
    ...
    def step2(self, closure=None):
        ...
        for group in self.param_groups:
            #logging.info('self params including %s' %(group))
            obj_and_grad_fn = self.obj_and_grad_fn2
            constraint_fn = self.constraint_fn
            for i, p in enumerate(group['params']):
                if p.grad is None:
                    continue
