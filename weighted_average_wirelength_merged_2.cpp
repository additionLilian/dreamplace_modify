...
int computeWeightedAverageWirelengthMergedLauncher(
  ...
  //        pin_count = 0;
  //        nets_count = nets_count + 1;
  //      }
  //    }    
  //  }
  int num_pins = netpin_start[num_nets-1] - netpin_start[0];
  //int rand_number = rand()%(nets_count);
  //int net_index[] = {net_list[rand_number], net_list[rand_number+1]};
  int nets_size = num_nets/2;
  //int rand_start = 30;
  int rand_start = rand()%(num_nets/2);
  int num_pins_between = netpin_start[rand_start + nets_size] - netpin_start[rand_start];
  ...
  //for (int i = net_index[0]; i < net_index[1]; ++i) {
    if (net_mask[i]) {
      ...
      partial_wl[i] = (xexp_x_sum / exp_x_sum - xexp_nx_sum / exp_nx_sum +
                      yexp_y_sum / exp_y_sum - yexp_ny_sum / exp_ny_sum)*1;
      ...
      for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j) {
        ...
        grad_intermediate_x[flat_netpin[j]] =
            ((a_x + b_x * xx) * exp_x - (a_nx + b_nx * xx) * exp_nx)*1;
        ...
        grad_intermediate_y[flat_netpin[j]] =
            ((a_y + b_y * yy) * exp_y - (a_ny + b_ny * yy) * exp_ny)*1;
      }
    }
  }
