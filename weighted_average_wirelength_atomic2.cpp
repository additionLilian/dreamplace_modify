...
int computeWeightedAverageWirelengthAtomicLauncher2(
    const T *pos, const int *pin2net_map, const int *flat_netpin,
    const int *netpin_start, const unsigned char *net_mask, int num_nodes, int num_pins, int num_nets,
    const T *inv_gamma, const int *flat_node2pin_map, const int* flat_node2pin_start_map, T *exp_xy, T *exp_nxy, T *exp_xy_sum,
    ...
    computepartialwl(x, y, exp_xy_sum, exp_nxy_sum, xy_max, xy_min, exp_xy, exp_nxy,
    pin2net_map, net_mask, flat_node2pin_start_map,flat_node2pin_map,//pins_array,
    num_nodes, num_pins, num_nets, partial_wl, num_threads);
  }
...
std::vector<at::Tensor> weighted_average_wirelength_atomic_forward2(
    at::Tensor pos, at::Tensor pin2net_map, at::Tensor flat_netpin,
    at::Tensor netpin_start, at::Tensor net_weights, at::Tensor net_mask,
    at::Tensor inv_gamma, at::Tensor flat_node2pin_map, at::Tensor flat_node2pin_start_map) {
  CHECK_FLAT_CPU(pos);
  CHECK_EVEN(pos);
  ...
  int num_nodes = flat_node2pin_start_map.numel();
  //int num_nodes = flat_node2pin_start_map.numel();
  ...
  at::Tensor partial_wl = at::zeros({num_pins}, pos.options());
  ...
  DREAMPLACE_DISPATCH_FLOATING_TYPES(
      pos, "computeWeightedAverageWirelengthAtomicLauncher2", [&] {
        computeWeightedAverageWirelengthAtomicLauncher2<scalar_t, V>(
            DREAMPLACE_TENSOR_DATA_PTR(pos, scalar_t),  // x then y
            DREAMPLACE_TENSOR_DATA_PTR(pin2net_map, int),
            DREAMPLACE_TENSOR_DATA_PTR(flat_netpin, int),
            DREAMPLACE_TENSOR_DATA_PTR(netpin_start, int),
            DREAMPLACE_TENSOR_DATA_PTR(net_mask, unsigned char), num_nodes, num_pins, num_nets,
...
/// equation
at::Tensor weighted_average_wirelength_atomic_backward2(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward",
        &DREAMPLACE_NAMESPACE::weighted_average_wirelength_atomic_forward2,
        "WeightedAverageWirelength forward");
  m.def("backward",
        &DREAMPLACE_NAMESPACE::weighted_average_wirelength_atomic_backward2,
